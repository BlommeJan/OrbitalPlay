...
function zvhUpdate(dt) {
	...
	if (zvhPlayer.x > zvhWidth - zvhPlayer.radius)
		zvhPlayer.x = zvhWidth - zvhPlayer.radius;
	if (zvhPlayer.y < zvhPlayer.radius) zvhPlayer.y = zvhPlayer.radius;
	if (zvhPlayer.y > zvhHeight - zvhPlayer.radius)
		zvhPlayer.y = zvhHeight - zvhPlayer.radius;
	...
	for (let i = zvhZombies.length - 1; i >= 0; i--) {
		...
		// Check collision
		if (
			Math.hypot(z.x - zvhPlayer.x, z.y - zvhPlayer.y) <
			z.radius + zvhPlayer.radius
		) {
			zvhPlayerHP -= 100 * dt;
...
function zvhDraw() {
	...
	// HUD
	document.getElementById('zvh-hpDisplay').textContent = `HP: ${Math.floor(
		zvhPlayerHP
	)}`;
	document.getElementById('zvh-waveDisplay').textContent = `Wave: ${zvhWave}`;
	document.getElementById(
		'zvh-scoreDisplay'
	).textContent = `Score: ${zvhScore}`;
...
function zvhOnKeyDown(e) {
	...
	if (e.key === 'Escape') {
		if (zvhPlayerHP <= 0) {
			...
			document.getElementById('zvh-pauseOverlay').style.display = zvhIsPaused
				? 'flex'
				: 'none';
		}
